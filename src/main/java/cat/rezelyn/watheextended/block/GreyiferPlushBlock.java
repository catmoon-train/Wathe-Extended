...
public class GreyiferPlushBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, WatheExtendedBlockEntities.GREYIFER_PLUSH, GreyiferPlushBlockEntity::tick);
    }
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            world.playSound(null, pos, RatatouilleSounds.BLOCK_RAT_MAID_PLUSH_HONK, SoundCategory.BLOCKS, 1.0F, pitch);
